...
let currentAddress = 'your current location'; // Store the resolved address
let lastAIGuide = '';
...
function getFreshLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
        function(position) {
            ...
            reverseGeocodeCoords(currentPosition, function(address) {
...
function displayAIGuide(guide) {
    lastAIGuide = guide; // Store the last AI guide output
    const chatWindow = document.getElementById('chat-window');
...
async function handleFollowupQuestion() {
    ...
    try {
        ...
        // Detect if translation is requested
        const translation_keywords = ['translate', 'hindi', 'hinglish', 'हिंदी', 'translate into hindi', 'hinglish mein'];
        const is_translation_request = translation_keywords.some(keyword => question.toLowerCase().includes(keyword));
        const body = {
            question: question,
        ...
        if (is_translation_request && lastAIGuide) {
            body.guide_to_translate = lastAIGuide;
        }
...
function reverseGeocodeCoords(coords, callback) {
    if (!window.google || !google.maps) {
        callback('your current location');
        return;
    }
